...
uint8_t Finger::attach(uint8_t dir0, uint8_t dir1, uint8_t dir2, uint8_t posSns)
{
	return attach(dir0, dir1, dir2, posSns, (uint8_t)-1, 0);			// no force sense, do not invert
}

// attach pins to a finger using only position control, but allow the direction to be inverted
uint8_t Finger::attach(uint8_t dir0, uint8_t dir1, uint8_t dir2, uint8_t posSns, bool inv)
{
	return attach(dir0, dir1, dir2, posSns, (uint8_t)-1, inv);		// no force sense
}

// attach pins to a finger using position control and force control, and allow the direction to be inverted
uint8_t Finger::attach(uint8_t dir0, uint8_t dir1, uint8_t dir2, uint8_t posSns, uint8_t forceSns, bool inv)
{
	...
	if ((_fingerIndex >= 0) && (_fingerIndex < MAX_FINGERS))
	{
		...
		pinMode(dir2, OUTPUT);		// set direction2 pin to output
		...
		setPWMFreq(dir2, 0x01);		// set PWM frequency to max freq
#endif
...
void Finger::motorControl(signed int speed)
{
	...
	if (dir == OPEN)
	{
		shiftWrite(_pin.dir[0], HIGH); //3 is MotB 2 is MotA here is (3,2)
		shiftWrite(_pin.dir[1], LOW);
	}
	else
	{
		shiftWrite(_pin.dir[0], LOW);
		shiftWrite(_pin.dir[1],HIGH);
	}

	// write the speed to the motors
	analogWrite(_pin.dir[2], speed);		//write motor speed to one direction pin
...
// shiftwrite for the register
void Finger::shiftWrite(int output, int high_low)
{
